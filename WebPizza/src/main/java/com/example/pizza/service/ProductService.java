...
//cdd
	...
	@Override
	public Page<Product> listAllSort(int pageNumber, String sortText, String keyword) {
		
		Pageable pageable;
		if(sortText !=null) {
			Sort sort = Sort.by("donGia");
			sort = sortText.equals("asc")?sort.ascending():sort.descending();
			pageable= PageRequest.of(pageNumber -1, 5,sort); 
		}else {
	...
	@Override
	public Page<Product> listSortByCategory(int pageNumber, Integer idCategory) {
			Pageable pageable;
			pageable = PageRequest.of(pageNumber -1, 5);
